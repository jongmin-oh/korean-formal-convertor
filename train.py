...
df = pd.read_excel("./data/formal_convertor_train.xlsx", index_col=0)
...
train_df["input"] = "존댓말로 바꿔주세요: " + train_df["informal"]
val_df["input"] = "존댓말로 바꿔주세요: " + val_df["informal"]

# 출력 문장의 끝에 "." 추가
train_df["output"] = train_df["formal"] + "."
val_df["output"] = val_df["formal"] + "."
...
class FormalTranslatorDataset(torch.utils.data.Dataset):
...
train_dataset = FormalTranslatorDataset(train_encodings, train_labels_encodings)
val_dataset = FormalTranslatorDataset(val_encodings, val_labels_encodings)
